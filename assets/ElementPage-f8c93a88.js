...
`,{white:si}=V.colors,oi=()=>{const e=ae(),{activeCluster:t}=te(),{allElements:n,activeElement:a,elementTrash:o,elementFilter:c,isLoading:r}=oe(),[s,h]=L.useState(si),[k,d]=L.useState(0),[m,y]=L.useState(!1),{ref:f,inView:T,entry:x}=cr({initialInView:!0,rootMargin:`0px 0px ${window.innerHeight/2}px 0px`});L.useEffect(()=>{if(!t)return;const i=()=>{const $=document.getElementById("active-element");$&&Ye($)};(a==null?void 0:a._id)===t.activeElementId?i():e(Dt({clusterId:t._id})).unwrap().then(({result:$})=>{if(!t.activeElementId)return;const E=$.elements.find(({_id:D})=>D===t.activeElementId);e(Oe(E))}).then(()=>i())},[]),L.useEffect(()=>{const i=E=>{E.altKey&&y(!0)},$=E=>{E.altKey||y(!1)};return addEventListener("keyup",$),addEventListener("keydown",i),()=>{removeEventListener("keydown",i),removeEventListener("keyup",$)}},[m,y]);let{elementSelect:l}=oe();l=l||[];const b=n.filter(i=>i.clusterId===(t==null?void 0:t._id)).sort((i,$)=>i.createdAt.localeCompare($.createdAt)),A=(()=>{const i=o.map($=>$._id);return l.includes("trash")?b.filter($=>i.includes($._id)):b})().filter(({element:i,caption:$,favorite:E,checked:D})=>{const F=i.toLowerCase().replace("·","").includes(c)||i.toLowerCase().includes(c)||$.toLowerCase().includes(c),W=l.includes("wordlist")?F&&Fn(i):F,M=()=>l.some(j=>["favorite","unfavorite"].includes(j))?l.includes("favorite")?W&&E===!0:W&&E===!1:W;return(()=>l.some(j=>["checked","unchecked"].includes(j))?l.includes("checked")?M()&&D===!0:M()&&D===!1:M())()}).sort(t!=null&&t.sortBy?(i,$)=>$.createdAt.localeCompare(i.createdAt):(i,$)=>i.createdAt.localeCompare($.createdAt)),v=window.innerHeight<(x==null?void 0:x.target.getBoundingClientRect().y)+(x==null?void 0:x.target.getBoundingClientRect().height)*1.5;return S.jsxs(S.Fragment,{children:[S.jsxs(ii,{id:"element-list",children:[A.map(i=>S.jsx(Zt,{el:i,liColor:s,setLiColor:h,editCount:k,setEditCount:d,selectMode:m},i._id)),!r&&S.jsxs("div",{ref:f,children:[S.jsx(zt,{}),S.jsx(Nt,{className:T&&v?"bar-up":"bar-down"}),S.jsx(qt,{className:T&&v?"bar-up":"bar-down",filtredElements:A,setLiColor:h})]})]}),S.jsxs(Xt,{children:[S.jsx("span",{children:"Translate"})," ",S.jsx(Pt,{size:18})]}),r&&S.jsx(Kn,{})]})},{s:_t,m:ai}=V.indents,fi=()=>S.jsx(He,{$p:`0 ${ai} ${_t} ${_t}`,children:S.jsx(oi,{})});export{fi as default};
