...
`,Xt=({children:e})=>{var y;const{selection:t,position:n,offset:a,nodeText:o}=Zn(),{openUrls:c}=Qn(),{allElements:r}=oe(),{activeCluster:s}=ee(),d=(y=r.find(f=>f.caption===o))==null?void 0:y.lang;console.log("captionLang: ",d);const k=s.lang;console.log("elementLang: ",k);const h=rr(t).map(f=>f[d||k]).filter(f=>f&&f);if((t==null?void 0:t.split(/\s+/).length)>1)return;const m=()=>{c(h)};if(t)return $.jsx(ei,{$x:(n==null?void 0:n.x)-20-a,$y:(n==null?void 0:n.y)-98,$selection:t,onClick:m,children:e})};Xt.propTypes={onClick:N.func,children:N.oneOfType([N.string,N.node,N.oneOf(["img","png","svg"])])};const{background:ti,backgroundHoverd:ni}=U.colors,ri=de`
  width: 100%;
  display: flex;
  align-items: center;
  justify-content: center;
...
`,{white:oi}=U.colors,ai=()=>{const e=ae(),{activeCluster:t}=ee(),{allElements:n,activeElement:a,elementTrash:o,elementFilter:c,isLoading:r}=oe(),[s,d]=L.useState(oi),[k,h]=L.useState(0),[m,y]=L.useState(!1),{ref:f,inView:S,entry:A}=lr({initialInView:!0,rootMargin:`0px 0px ${window.innerHeight/2}px 0px`});L.useEffect(()=>{if(!t)return;const i=()=>{const T=document.getElementById("active-element");T&&qe(T)};(a==null?void 0:a._id)===t.activeEl?i():e(Ot({cluster:t._id})).unwrap().then(({result:T})=>{if(!t.activeEl)return;const C=T.elements.find(({_id:D})=>D===t.activeEl);e(Oe(C))}).then(()=>i())},[]),L.useEffect(()=>{const i=C=>{C.altKey&&y(!0)},T=C=>{C.altKey||y(!1)};return addEventListener("keyup",T),addEventListener("keydown",i),()=>{removeEventListener("keydown",i),removeEventListener("keyup",T)}},[m,y]);let{elementSelect:u}=oe();u=u||[];const b=n.filter(i=>i.cluster===(t==null?void 0:t._id)).sort((i,T)=>i.createdAt.localeCompare(T.createdAt)),x=(()=>{const i=o.map(T=>T._id);return u.includes("trash")?b.filter(T=>i.includes(T._id)):b})().filter(({element:i,caption:T,favorite:C,checked:D})=>{const F=i.toLowerCase().replace("·","").includes(c)||i.toLowerCase().includes(c)||T.toLowerCase().includes(c),W=u.includes("wordlist")?F&&qn(i):F,I=()=>u.some(j=>["favorite","unfavorite"].includes(j))?u.includes("favorite")?W&&C===!0:W&&C===!1:W;return(()=>u.some(j=>["checked","unchecked"].includes(j))?u.includes("checked")?I()&&D===!0:I()&&D===!1:I())()}).sort(t.sortBy?(i,T)=>T.createdAt.localeCompare(i.createdAt):(i,T)=>i.createdAt.localeCompare(T.createdAt)),v=window.innerHeight<(A==null?void 0:A.target.getBoundingClientRect().y)+(A==null?void 0:A.target.getBoundingClientRect().height)*1.5;return $.jsxs($.Fragment,{children:[$.jsxs(si,{children:[x.map(i=>$.jsx(Qt,{el:i,liColor:s,setLiColor:d,editCount:k,setEditCount:h,selectMode:m},i._id)),!r&&$.jsxs("div",{ref:f,children:[$.jsx(zt,{}),$.jsx(Nt,{className:S&&v?"bar-up":"bar-down"}),$.jsx(Yt,{className:S&&v?"bar-up":"bar-down",filtredElements:x,setLiColor:d})]})]}),$.jsxs(Xt,{children:[$.jsx("span",{children:"Translate"})," ",$.jsx(It,{size:18})]}),r&&$.jsx(Kn,{})]})},{s:_t,m:ci}=U.indents,hi=()=>$.jsx(He,{$p:`0 ${ci} ${_t} ${_t}`,children:$.jsx(ai,{})});export{hi as default};
